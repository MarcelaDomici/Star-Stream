...
public class CommentScreen {
            ...
            scene.addEventHandler(KeyEvent.KEY_PRESSED, (key) -> {
                if(key.getCode()==KeyCode.ENTER) {
                    if(this.txtNewComment.getText().length()!=0){
                        ...
                        if(hyperPost!=null)hyperPost.setText("Comentários "+String.valueOf(post.getComments().size()));
        ...
        if (!post.getComments().isEmpty()) {
            for(int i =post.getComments().size()-1;i>=0;--i){
                ...
                if(userNow.checkFriend(indexF) == 0){
                    ...
                    ImageView imgIConFriend = (user[post.getComments().get(i).getIdUser()].getPhotoProfile()==null)?new ImageView(new Image(getClass().getResourceAsStream("ScreensFXML/Imagens/PERFIL.png")))
                    :
                    new ImageView(new Image(new FileInputStream(user[post.getComments().get(i).getIdUser()].getPhotoProfile())));
                    ...
                    if(List_User.getPoint(2).user[id].getSolicit().contains((Integer)indexF) || List_User.getPoint(2).user[id].getList_Solicit().contains((Integer)indexF)){
    ...
    private void doLike(MouseEvent event) {
        if(post.checkLike(id)!=0){
            ...
            post.addLike(id);
            if(otherButton!=null)this.otherButton.setStyle(btnLike.getStyle());
          }else{
            btnLike.setStyle(
                "-fx-shape: \"M11.8,0c-1.7,0-3.2,1.4-3.8,2.8C7.4,1.4,5.9,0,4.2,0C1.9,0,0,1.9,0,4.2c0,4.7,4.8,5.9,8,10.63.0-4.65,8-6.05,8-10.6C16,1.9,14.1,0,11.8,0z\"; -fx-text-fill: black;"
            );
        ...
        if(qntLike!=null)qntLike.setText(String.valueOf(this.post.getLikes()));
    }
